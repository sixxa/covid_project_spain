...
def fit_forecast(
    series: pd.DataFrame, end_train: str, future_dates: pd.DatetimeIndex
) -> Tuple[pd.Series, pd.Series, pd.Series]:
    cap = series["y"].max() * 2
    train = series[series["ds"] <= end_train].copy()
    train["floor"] = 0.0
    train["cap"] = cap
    ...
    future_df = pd.DataFrame({"ds": future_dates, "floor": 0.0, "cap": cap})
    forecast = model.predict(future_df)
    return (
        forecast["yhat"].clip(lower=0),
...
def main() -> None:
    ...
    pt_short, lo_short, hi_short = fit_forecast(sp, "2022-12-31", idx)
    pt_full, lo_full, hi_full = fit_forecast(sp, "2024-12-31", idx)
    ...
    fig.update_layout(
        ...
        title="Prophet 2025 Forecast: Sensitivity to Training Window",
        font={"size": 18},
        legend={"orientation": "h", "y": -0.2},
    )
    fig.update_yaxes(title="Predicted daily cases", rangemode="tozero")
